...
constexpr float hex_width   = sqrt(3)*hex_size;
constexpr float hex_height  = 2.0f*hex_size;
...
static std::pair<float, float> to_screen(int q, int r){
	return {r*2*hex_size*axial::y1 + q*hex_width*axial::x1, r*2*hex_size*axial::y2};
}
...
static std::pair<int,int> closest_hex(float x, float y){
	y = 600.0f - y;
	x -= 800.0f/2.0f;
	y -= 600.0f/2.0f;
	x /= 1.05f;
